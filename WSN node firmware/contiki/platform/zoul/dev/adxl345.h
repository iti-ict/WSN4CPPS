...
/**
 * \file
 *         Device drivers header file for adxl345 accelerometer in Zolertia RE-Mote
 * \author
 *         Rubén Ponce, ITI <rponce@iti.es>
 */
/*---------------------------------------------------------------------------*/
#ifndef ADXL345_H_
#define ADXL345_H_
...
/*Set on/off the accelerometer adxl345
*If value is 1, accelerometer turn on
*else if value is 0 accelerometer turn off
*/
void accm_set_pin_cs(uint8_t port, uint8_t pin, uint8_t value);

/* Write registers in the adxl345 accelerometer. 
*The table of the registers are in the Datasheet
*/
void accm_write_reg(uint8_t reg, uint8_t val);

/*Read registers from the adxl345. You can check the registers which you can find
* in the datasheet
*/
 ...
 * so is wanted/needed.
 */
uint16_t accm_read_axis(enum ADXL345_AXIS axis);
...
#define ADXL345_DATAY1          0x35  /* read only, MSByte Y */
#define ADXL345_DATAZ0          0x36  /* read only, LSByte Z */
#define ADXL345_DATAZ1          0x37  /* read only, MSByte Z */
...
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
#endif /* ifndef ADXL345_DRIVERS_H_ */
